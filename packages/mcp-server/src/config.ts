...
function validateToken(source: string, raw: string): string {
  ...
  throw new ConfigError(
    ...
    `Set ${TOKEN_ENV_VAR} to the token value alone — no "Bearer " prefix, no quotes, no trailing newline — or run \`mneia login\` to write a fresh one.`,
  );
}
...
async function resolveToken(env: EnvLike, readTextFile: FileReader): Promise<string> {
  ...
  if (fromEnv !== undefined) {
    throw new ConfigError(
      ...
      `Set ${TOKEN_ENV_VAR} to a Mneia token in the MCP client's server config, or unset it and run \`mneia login\` to approve this machine in the browser.`,
    );
  }
  ...
  try {
    ...
    if (isNotFound(cause)) {
      throw new ConfigError(
        TOKEN_ENV_VAR,
        `no Mneia credentials were found: ${TOKEN_ENV_VAR} is unset and ${path} does not exist.`,
        `Run \`mneia login\` to approve this machine, or set ${TOKEN_ENV_VAR} in the MCP client's server config for a non-interactive environment such as CI.`,
      );
    }
    throw new ConfigError(
      TOKEN_ENV_VAR,
      `${path} could not be read: ${describeCause(cause)}.`,
      `Check the file permissions and run \`mneia login\` again, or set ${TOKEN_ENV_VAR} instead.`,
    );
  }
  ...
  if (raw.trim().length === 0) {
    throw new ConfigError(
      ...
      `Run \`mneia login\` to write a token, or set ${TOKEN_ENV_VAR} instead.`,
    );
  }
...
async function loadProjectBinding(
  ...
  try {
    ...
    throw new ConfigError(
      path,
      `${path} could not be read: ${describeCause(cause)}.`,
      'Check the file permissions on .mneia/config.json, or delete it and run `mneia init` again.',
    );
  }
  ...
  try {
    ...
    throw new ConfigError(
      ...
      'Delete the file and run `mneia init` to rewrite it, or pass the project name to each tool call instead.',
    );
  }
  ...
  if (!result.success) {
    throw new ConfigError(
      ...
      'Fix the field named above, or delete the file and run `mneia init` to rewrite it.',
    );
  }
