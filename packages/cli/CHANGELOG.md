...
- 55a6284: `mneia init` now installs the end-of-turn checkpoint hook, not only session-start rehydration.
...
- ed4d4bb: `mneia checkpoint` with no `--session` now checkpoints only the harness sessions active in the
...
- d0a3c74: Add guided MCP client installation, detection, listing, and removal with complete setup prompts for supported coding agents.
