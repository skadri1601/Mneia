...
leaders responsible for enforcement using **GitHub's private vulnerability reporting** on this
repository, which is currently the only monitored private channel.

> **A dedicated conduct address is not yet live.** Reserving the domain is tracked separately and is
> not complete, so publishing an address here would route reports nowhere. Until it exists, use the
> GitHub channel above — it is monitored. This note gets removed, and a real address added, when the
> domain lands.
